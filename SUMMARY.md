# Table of contents

* [Today I learned!](README.md)
* [Deployment](deployment/README.md)
  * [Rolling, Canary, Blue-green deployments](deployment/canary-releases.md)
  * [Kubernetees Helm Charts](deployment/kubernetees-helm-charts.md)
* [AI/ML](ai-ml/README.md)
  * [SeldonIO](ai-ml/seldonio.md)
  * [Better to pass callback in fit\_one\_cycle](ai-ml/better-to-pass-callback-in-fit\_one\_cycle.md)
  * [Eliza - demo](ai-ml/eliza-demo.md)
  * [Helsinki-NLP Translation models](ai-ml/helsinki-nlp-translation-models.md)
* [Fastai Learning](fastai-learning.md)

## Python

* [Understanding get\_image\_files in fastai](python/untitled-1-1.md)
* [Resizing an image to smaller size](python/untitled-1.md)
* [Extracting a Json Object as List(regex)](python/extracting-a-json-object-as-list-regex.md)
* [f-strings debugging shortcut](python/f-strings-debugging-shortcut.md)
* [Pytest](python/pytest.md)
* [conda switch between python versions](python/conda-switch-between-python-versions.md)
* [Nested functions exception handling](python/nested-functions-exception-handling.md)

## Programming

* [Certbots usage](programming/certbots-usage.md)
* [Code highlighting in Google Docs](programming/code-highlighting-in-google-docs.md)
* [HTTP Methods](programming/http-methods.md)
* [How to use vertical mouse?](programming/how-to-use-vertical-mouse.md)
* [HTTP Status Codes](programming/http-status-codes.md)
* [Keycloak, Oauth, OpenID connect, SAML](programming/keycloak-oauth-openid-connect-saml.md)
* [Why should NPM packages be as small as possible?](programming/untitled-1.md)
* [Clean Architecture](programming/clean-architecture.md)
* [REST vs gRPC](programming/rest-vs-grpc.md)
* [Keycloak concepts](programming/keycloak-concepts.md)
* [what is proxy server and nginx?](programming/what-is-proxy-server-and-nginx.md)
* [Asymptotic Time Complexity](programming/asymptotic-time-complexity.md)

## async/await

* [JavaScript Asynchronous operation](async-await/javascript-asynchronous-operation.md)
* [Lesson 2- Eventloops](async-await/lesson-2-eventloops.md)
* [Lesson 1- asyncio history](async-await/lesson-1-asyncio-history.md)
* [Lesson 3- using coroutines](async-await/lesson-3-using-coroutines.md)
* [Lesson 4- coroutines in hood](async-await/lesson-4-coroutines-in-hood.md)
* [Python async/await](async-await/python-async-await.md)
* [JavaScript](async-await/javascript.md)

## R Programming language

* [Facet\_grid and Facet\_wrap](r-programming-language/facet\_grid-and-facet\_wrap.md)
* [geom\_point](r-programming-language/geom\_point.md)

## C programming language

* [Inputting String in C programming language](c-programming-language/inputting-string-in-c-programming-language.md)
* [Checking if a element is passed as input or not?](c-programming-language/checking-if-a-element-is-passed-as-input-or-not.md)

## Git/Github

* [give credits to other people](git-github/give-credits-to-other-people.md)
* [one time setting to easily use Github](git-github/one-time-setting-to-easily-use-github.md)
* [Checkout to specific tag](git-github/checkout-to-specific-tag.md)
* [git suggestions in PR](git-github/git-suggestions-in-pr.md)
* [Using emojis in git commits](git-github/using-emojis-in-git-commits.md)

## Databases

* [Postgres Database Dockercompose](databases/postgres-database-dockercompose.md)
* [TIL New SQL Operators - Except, UNION, Distinct](databases/til-new-sql-operators-except-union-distinct.md)
* [Analysing Performance of DB Queries](databases/analysing-performance-of-db-queries.md)
* [Querying Date ranges in postgres](databases/querying-date-ranges-in-postgres.md)
* [Handling Database disconnects in SQLAlchemy](databases/handling-database-disconnects-in-sqlalchemy.md)

***

* [WITH NO EXCEPT](with-no-except.md)
* [What is difference with JSON documents in postgres and Mongodb](what-is-difference-with-json-documents-in-postgres-and-mongodb.md)
